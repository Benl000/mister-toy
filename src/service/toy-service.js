...
function getById(id) {
  return axios.get(BASE_URL + TOY_URL + id).then((res) => res.data);
}

function remove(id) {
  return axios.delete(BASE_URL + TOY_URL + id);
}

function save(toy) {
  const toyToSave = JSON.parse(JSON.stringify(toy));
  const savedToy = toyToSave._id
    ? axios.put(BASE_URL + TOY_URL + toy._id, toy)
    : axios.post(BASE_URL + TOY_URL, toy);
  return savedToy;
}
